 ...
 *
 * **Cancella quello che trova**: serve a mostrare il sito pieno, non ad
 * aggiungersi a un catalogo vero. La usano sia `pnpm db:seed` sia la rotta di
 * installazione, che per questo la chiede con una parola esplicita.
 */
...
export type EsitoEsempi = { collezioni: number; prodotti: number }

export async function seminaEsempi(): Promise<EsitoEsempi> {
  ...
  return { collezioni: COLLEZIONI.length, prodotti: PRODOTTI.length }
}
